...
export default class Publish extends Component {
  ...
  onSubmit(event) {
    ...
    ipfs.files.add(this.state.buffer, (error, result) => {
      ...
      photoNFTFactory.methods.createNewPhotoNFT(nftName, nftSymbol, photoPrice, ipfsHashOfPhoto, nftOwner).send({ from: accounts[0] })
        .once('receipt', async (receipt) => {
          ...
          await photoNFTMarketPlace.methods.openTradeWhenCreateNewPhotoNFT(await PHOTO_NFT, photoId, photoPrice, nftOwner).send({ from: accounts[0] })
          console.log("owner of photo after transfer ", photoNFT.methods.ownerOf(photoId).call())
